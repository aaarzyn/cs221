...
def numpy_cross_entropy_loss(predictions: np.ndarray,
    ...
    ce_loss = -np.sum(targets * np.log(preds)) / targets.shape[0]
...
def numpy_compute_gradients(features: np.ndarray, predictions: np.ndarray,
    ...
    grad_weights = features.T @ error / batch_size
...
def predict_linear_classifier(features: np.ndarray, labels: np.ndarray, weights: np.ndarray,
    ...
    return np.mean(np.argmax(features @ weights + bias, axis=1) == np.argmax(labels, axis=1))
    # END_YOUR_CODE
...
def train_linear_classifier(train_features: np.ndarray, train_labels: np.ndarray,
    ...
    for epoch in range(num_epochs):
        ...
        preds = numpy_softmax(logits)

        loss = numpy_cross_entropy_loss(preds, train_labels)
        grad_weights, grad_bias = numpy_compute_gradients(train_features, preds, train_labels)
...
def text_to_average_embedding(text: str, vocab: Vocabulary,
    ...
    embeds = embedding_layer(indices_tensor)
...
def extract_averaged_features(texts: List[str], vocab: Vocabulary,
    ...
    """
    # BEGIN_YOUR_CODE (our solution is 5 lines of code, but don't worry if you deviate from this)
    features = [text_to_average_embedding(text, vocab, embedding_layer) for text in texts]
...
def torch_cross_entropy_loss(predictions: torch.Tensor,
    ...
    loss = -(targets * torch.log(predictions)).sum(dim=1).mean()
    return loss
    # END_YOUR_CODE
...
def train_mlp_classifier(
    ...
    for epoch in range(num_epochs):
        ...
        indexes = np.random.permutation(len(train_texts))
        losses = []
        ...
        accuracy = predict_mlp(val_texts, val_labels, classifier, embedding_layer, vocab)
        print(f"Epoch {epoch + 1}: Train Loss={train_loss:.2f}, Accuracy={accuracy:.2f}")
